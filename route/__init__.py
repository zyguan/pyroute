...
def distance(lng1, lat1, lng2, lat2, formula=VINCENTY, iterlimit=20):
    ...
    :return: the distance between (lng1, lat1) and (lng2, lat2) in meter
    """
    return _route.distance(lng1, lat1, lng2, lat2, formula, iterlimit)


def distance_vincenty(lng1, lat1, lng2, lat2):
    return _route.distance(lng1, lat1, lng2, lat2, VINCENTY)


def distance_haversine(lng1, lat1, lng2, lat2):
    return _route.distance(lng1, lat1, lng2, lat2, HAVERSINE)
...
def measure(ps, formula=VINCENTY, iterlimit=20):
    ...
    distance from (lng_i, lat_i) to (lng_j, lat_j) in meter.
    """
